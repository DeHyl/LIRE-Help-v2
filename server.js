// ─── LIRE Help — Express Server ───────────────────────────────────────────────
// Serves static files + /api/chat endpoint for tenant concierge.
// Railway deploy: auto-detects package.json → runs `npm start`
...
app.post('/api/chat', async (req, res) => {
  try {
    ...
    // Log session (in production, persist to DB)
    if (sessionId) {
...
app.listen(PORT, () => {
  console.log(`LIRE Help running on port ${PORT}`);
});
